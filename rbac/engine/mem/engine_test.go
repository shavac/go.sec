...
func TestGetDropRole(t *testing.T) {
	e:= Init()
	_, ceoid:=e.GetRole("ceo",true)
	e.GrantRole("ceo","hr_mgr")
	e.GrantRole("ceo","acct_mgr")
	if exist, id := e.GetRole("ceo", false); ! exist || id != ceoid {
		t.Fatal("ceo not created")
	}
	if exist, _ := e.GetRole("hr_mgr", false); ! exist {
...
func TestGrantRevokeDropPerm(t *testing.T) {
	...
	if e.RBACDecision("ceo","employee","delete") {
	...
	if ! e.RBACDecision("ceo","employee","select") {
...
func TestRBACDecision(t *testing.T) {
	...
	e.GrantPerm("hr_stf", "employee", "select", "insert")
	e.GrantPerm("acct_mgr", "employee", "update")
	if ! e.RBACDecision("ceo", "employee", "insert", "delete", "update", "select") {
		t.Fatal("ceo should has permission of update employee")
	}
	if !e.RBACDecision("hr_mgr", "employee", "select", "delete", "update", "insert") {
		t.Fatal("hr_mgr should has permission of idus employee")
	}
	if e.RBACDecision("hr_stf", "employee", "insert", "delete", "update", "select") {
...
func TestRBAC2Decision(t *testing.T) {
	...
	if ! e.RBAC2Decision("admin","http://sina.com/sports/nba","UPLOAD","REMOVE","GET") {
		t.Fatal("admin should have all permission")
	}
	if ! e.RBAC2Decision("sports_admin","http://sina.com/finance/fond","GET") {
		t.Fatal("sports_admin should have GET any page permission")
	}
	if ! e.RBAC2Decision("sports_admin","http://sohu.com","GET") {
		t.Fatal("sports_admin should have GET any page permission")
	}
	if e.RBAC2Decision("admin","http://sina.com","DOWNLOAD") {
